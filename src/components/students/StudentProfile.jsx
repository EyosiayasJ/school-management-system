...
const StudentProfile = ({ student, onClose, onUpdateStudent, branchesList }) => {
  ...
  const handleWarn = () => {
    ...
    toast.success(`Warning issued to ${student.name}`);
    setActionModal(null);
  };
  ...
  const handleSuspend = (days) => {
    ...
    onUpdateStudent(updatedStudent);
    toast.success(`${student.name} suspended for ${days} days`);
    setActionModal(null);
  };
  ...
  const handleBlacklist = () => {
    ...
    onUpdateStudent(updatedStudent);
    toast.success(`${student.name} has been blacklisted`);
    setActionModal(null);
  };
  ...
  const handleExpel = () => {
    ...
    onUpdateStudent(updatedStudent);
    toast.success(`${student.name} has been expelled`);
    setActionModal(null);
    onClose();
  };
      ...
      >
        ...
        >
          ...
          <div className="bg-blue-600 text-white p-6 flex items-center">
            {student.avatar ? (
              <img src={student.avatar} alt="" className="h-20 w-20 rounded-full object-cover border-2 border-white" />
            ) : (
            ...
            <div className="ml-4">
              ...
              <p className="text-blue-100">{student.grade} Grade</p>
              <div className="flex mt-2 space-x-2">
                <span className="px-2 py-1 bg-blue-700 rounded-full text-xs">{student.branch}</span>
          ...
          <div className="p-6 overflow-y-auto" style={{ maxHeight: 'calc(80vh - 200px)' }}>
            {activeTab === 'info' ? (
              <div className="space-y-4">
                ...
                <InfoItem label="GPA" value={student.gpa || 'N/A'} />
                <InfoItem label="Branch" value={student.branch} />
                <InfoItem label="Status" value={student.status} />
                {student.status === 'suspended' && student.suspensionEnds && (
                  <InfoItem 
                    label="Suspension Ends" 
                    value={new Date(student.suspensionEnds).toLocaleDateString()} 
                  />
                )}
                <InfoItem 
                  label="Warnings" 
                  value={student.warnings || 0} 
                />
              </div>
            ) : (
              <div className="space-y-6">
                <div className="bg-yellow-50 p-4 rounded-md">
                  ...
                  <p className="mt-1 text-sm text-yellow-700">
                    Current warnings: <span className="font-bold">{student.warnings || 0}</span>
                  </p>
                  {student.status === 'suspended' && student.suspensionEnds && (
                    <p className="mt-1 text-sm text-yellow-700">
                      Suspended until: <span className="font-bold">{new Date(student.suspensionEnds).toLocaleDateString()}</span>
                    </p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <ActionButton 
                    ...
                    disabled={student.status === 'blacklisted' || student.status === 'expelled'}
                  />
                  <ActionButton 
                    label="Suspend" 
                    color="orange" 
                  ...
                  <ActionButton 
                    ...
                    disabled={student.status === 'expelled'}
                  />
                </div>
              </div>
            )}
...
const WarningModal = ({ onConfirm, onCancel }) => (
  <div>
    ...
    <p className="mt-2 text-sm text-gray-500">
      Are you sure you want to issue a warning to this student? This will be recorded in their disciplinary record.
    </p>
...
const SuspendModal = ({ onConfirm, onCancel }) => {
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-gray-500">
        For how many days would you like to suspend this student?
      </p>
      <div className="mt-4">
...
const BlacklistModal = ({ onConfirm, onCancel }) => (
  <div>
    <h3 className="text-lg font-medium text-gray-900">Blacklist Student</h3>
    <p className="mt-2 text-sm text-gray-500">
      Are you sure you want to blacklist this student? This will prevent them from being enrolled at any branch in the future.
    </p>
    <div className="mt-4 flex justify-end space-x-3">
      <button
        onClick={onCancel}
...
const ExpelModal = ({ onConfirm, onCancel }) => (
  <div>
    <h3 className="text-lg font-medium text-gray-900">Expel Student</h3>
    <p className="mt-2 text-sm text-gray-500">
      Are you sure you want to expel this student? This action cannot be undone.
    </p>
    <div className="mt-4 flex justify-end space-x-3">
      <button
        onClick={onCancel}
